...
Every request from the public internet is terminated at **Cloudflare's edge** (Layer 8). `cloudflared` runs on each origin and dials outbound, so the origins have **no public inbound app ports**. Cloudflare Access Free tier enforces email-domain allowlists before a single byte reaches the app. TLS is handled by Cloudflare — Caddy inside the cluster is ClusterIP-only.

## 🏗️ Domain 2: Infrastructure

- **Layer 1 — Cilium**: eBPF-based network identity. Default-deny ingress and egress. Every connection whitelisted by CNP.
...
- **Layer 7 — Splunk**: Single pane of glass for audit. Every process, connection, and file access flows here via Fluent Bit + OTel.

## 👩‍💻 Domain 3: Development workflow

- **Layer 5 — Cisco AI Defense**: AIBOM on every PR, Adversarial Hubness Detector on PDF changes, IDE scanner locally.
