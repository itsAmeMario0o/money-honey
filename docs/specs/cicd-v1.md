...
CLAUDE.md Layer 6 defines four CI/CD workflows that gate every code change before it reaches the cluster. `.github/workflows/quality.yaml` (from earlier work) already runs lint/tests/scans on every PR. This spec adds the four workflows that build images, deploy, and run Cisco AI Defense gates.
...
| FR-13 | It MUST invoke Cisco AI Defense `cisco-aibom` (installed from PyPI via `uv tool install cisco-aibom[agentic,llm-openai]`) to produce an AIBOM. The AIBOM is distinct from the SBOM that Trivy generates in `quality.yaml`: AIBOM catalogs AI-specific assets (models, agents, tools, MCP servers, embeddings, vector stores, datasets, prompts, guardrails, secrets), SBOM catalogs all deps. Both are required. They are NOT interchangeable. |
| FR-14 | `cisco-aibom analyze` is called with `--llm-model gpt-4o-mini --llm-provider openai`. `OPENAI_API_KEY` is a repo secret scoped to this workflow only. It is for the AIBOM tool's internal classifier, not the chatbot's LLM. When the secret is unset, the job emits a skip warning and exits 0 (advisory via `continue-on-error: true` until a baseline AIBOM is approved for diff-based blocking). |
| FR-14a | The SBOM equivalent is generated by Trivy in `quality.yaml trivy-fs`: CycloneDX JSON, uploaded as the `sbom-cyclonedx` artifact (30-day retention). Complements the AIBOM; does not substitute for it. |
...
| `OPENAI_API_KEY` | `aibom.yaml` | LLM for the Cisco AIBOM tool's internal classifier. Scoped to this workflow; NOT used by the runtime chatbot. |
