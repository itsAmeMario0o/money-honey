...
- Worker nodes: 3x `Standard_B2s` (originally planned `Standard_B2als_v2`; switched at apply time because eastus `standardBasv2Family` quota was exhausted)
...
| EC-9 | Two operators apply from different IPs in sequence | The NSG rule toggles to whichever operator ran `apply` most recently. Only one operator has SSH access at a time. |
...
- ✅ Private key in state. Yes, the `tls_private_key.private_key_pem` attribute lives in Terraform state. Mitigations: (1) state is in an encrypted Azure Blob with access-controlled, (2) local file copy is `0600` and gitignored, (3) state is the authoritative backup for recovery (EC-10). Acceptable for a demo; revisit if we ever handle real prod secrets.
