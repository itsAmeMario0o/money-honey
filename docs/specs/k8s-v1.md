...
No images exist in GHCR yet. That is step 5. This spec covers manifest authoring plus applying the subset that does not require runtime secrets or container images: network policies, tracing policies, SecretProviderClass shells, the `money-honey` namespace.
...
| FR-10 | A `CiliumNetworkPolicy` MUST allow `cloudflared` pods to reach Caddy (port 80). |
| FR-11 | A `CiliumNetworkPolicy` MUST allow Caddy to reach FastAPI (8000) and React (3000). |
| FR-12 | A `CiliumNetworkPolicy` MUST allow FastAPI to reach the Claude API (egress to `api.anthropic.com`, CIDR-based since we don't have FQDN filtering without ACNS). |
| FR-13 | A `CiliumNetworkPolicy` MUST allow FastAPI + Fluent Bit to reach Splunk HEC at `10.0.4.0/28:8088` (the `splunk` subnet CIDR). |
...
| NFR-2 (security) | No Deployment MAY use `hostNetwork`, `hostPID`, or `privileged: true`. Tetragon's DaemonSet (already deployed via Helm) is exempt because it needs kernel access. | 0 exceptions in `money-honey` ns |
...
| EC-3 | Container image not in GHCR yet | Pod stuck in `ImagePullBackOff`. This is expected before step 5 builds the images. Don't apply app Deployments until then. |
