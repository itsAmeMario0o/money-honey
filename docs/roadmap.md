...
- ChatOps via the Webex bot. Make `money-honey-ci` interactive: register a Webex webhook so the bot can receive messages, parse commands (`status`, `pods`, `deploy`, `splunk query`, `costs`, `help`), and respond in the space. Listener runs as a lightweight Python container in AKS (behind the Cloudflare tunnel) or as a GitHub Actions `repository_dispatch` relay. Needs a spec at `docs/specs/chatops-v1.md` before implementation. Blocked on Cloudflare tunnels being live (the bot needs a reachable webhook endpoint).
